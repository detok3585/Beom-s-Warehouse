...
## Lesson 1: Using the RxLocalParallel compute context with rxExec
...
- **foreach** is a popular way to parallelize base R across cores on a single computer

- The **doRSR** package enables you to use **rxExec** as a back end for the **%dopar%** syntax

  - Simplifies sharing across platforms

  - Enables you to use **%dopar%** on a remote server or cluster
...
- With long HPC jobs, you might prefer to send the job to the server to run in the background
  ...
  - Check back on the progress of the job using **rxGetJobStatus**  

  - Retrieve the results of the completed job with **rxgetJobResults** 
